...
visited = { cave:0 for cave in adj if is_small(cave) }
curr = "start"
assert is_small(curr)
visited[curr] += 1
visited_something_twice = False
curr_path = [curr]
...
while True:
    ...
    if nxt == 0:  # "Pop" marker?
        if is_small(curr):
            if visited[curr] == 2:
                assert visited_something_twice
                visited_something_twice = False
            visited[curr] -= 1
        ...
        if len(curr_path) == 0:
            assert curr == "start"
            assert s == []
            assert not visited_something_twice
            break
        curr = curr_path[-1]
        continue

    if is_small(nxt) and visited[nxt] > 0 and visited_something_twice:
        continue
    if nxt == "start":
        continue
